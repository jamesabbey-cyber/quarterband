...
def fetch_hourly_candles(pair, days=30, gran=3600):
    ...
    while remaining > 0:
        ...
        remaining -= take
    # Coinbase returns most-recent first typically; sort ascending by time
...
def hourly_seasonality(pair, days=30, gran=3600):
    ...
    try:
        ...
        # Normalize within each day
        acc = {h: [] for h in range(24)}
        for dkey, rows in by_day.items():
            # Ensure hours are unique-ish; aggregate same-hour duplicates by last value
            hour_to_close = {}
            for h, c in rows:
                hour_to_close[h] = c
            vals = list(hour_to_close.values())
            if len(vals) < 6:  # too sparse
                continue
        ...
        # Pick 2-hour windows around extrema (centered on min/max hour)
        ...
        return {
            ...
            "buy_hours": neighbors(min_h),   # hours where price tends to be relatively low
            "sell_hours": neighbors(max_h),  # hours where price tends to be relatively high
        }
    except Exception:
        return None
